...
class MF(BaseColdStartTrainer):
    ...
    def train(self):
        ...
        if self.args.save_emb:
            torch.save(self.user_emb, f"{self.args.save_root}/{self.args.dataset}_cold_{self.args.cold_object}_{self.args.model}_user_emb.pt")
            torch.save(self.item_emb, f"{self.args.save_root}/{self.args.dataset}_cold_{self.args.cold_object}_{self.args.model}_item_emb.pt")
    ...
    def load_emb(self, root, model_name):
        print(f"Loading pre-trained embeddings from {root} with prefix {model_name}")
        self.model.load_emb(root, model_name, self.device)
        with torch.no_grad():
            self.user_emb, self.item_emb = self.model()
...
class Matrix_Factorization(nn.Module):
    ...
    def load_emb(self, root, model_name, device):
        ...
        user_emb = torch.load(user_emb_path, map_location=device)
        item_emb = torch.load(item_emb_path, map_location=device)
        
        self.embedding_dict['user_emb'].data.copy_(user_emb)
        self.embedding_dict['item_emb'].data.copy_(item_emb)
