...
class LightGCN(BaseColdStartTrainer):
    def __init__(self, args, training_data, warm_valid_data, cold_valid_data, all_valid_data,
        ...
        super(LightGCN, self).__init__(args, training_data, warm_valid_data, cold_valid_data, all_valid_data,
                                       warm_test_data, cold_test_data, all_test_data, user_num, item_num,
                                       warm_user_idx, warm_item_idx, cold_user_idx, cold_item_idx, device)
    ...
    def train(self):
        ...
        if self.args.save_emb:
            torch.save(self.user_emb, f"./model_weight/{self.args.dataset}/base_model/{self.args.model}_{self.args.LLM_type}_user_emb.pt")
            torch.save(self.item_emb, f"./model_weight/{self.args.dataset}/base_model/{self.args.model}_{self.args.LLM_type}_item_emb.pt")
