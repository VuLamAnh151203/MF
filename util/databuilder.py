...
class ColdStartDataBuilder(object):
    def __init__(self, training_data, warm_valid_data, cold_valid_data, overall_valid_data,
        ...
        # Pre-register all users and items from the indices to ensure they are in the ID table
        for u in warm_user_idx:
            if u not in self.user:
                self.user[u] = len(self.user)
                self.id2user[self.user[u]] = u
                if self.source_user_content is not None:
                    self.mapped_user_content[self.user[u]] = self.source_user_content[u]
        for u in cold_user_idx:
            if u not in self.user:
                self.user[u] = len(self.user)
                self.id2user[self.user[u]] = u
                if self.source_user_content is not None:
